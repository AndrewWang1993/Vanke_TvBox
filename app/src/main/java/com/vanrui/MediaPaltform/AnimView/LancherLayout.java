package com.vanrui.MediaPaltform.AnimView;
...
import com.vanrui.MediaPaltform.Util.*;
...
public class LancherLayout extends LinearLayout implements ShadowCallBack, View.OnClickListener, View.OnFocusChangeListener {
	...
	public LancherLayout(Context paramContext) {
		...
		width = util.dip2px(context, 247);// 放大前的宽
		height = util.dip2px(context, 357);// 放大前的高
...
//		setOrientation(LinearLayout.HORIZONTAL);
	...
	public void initView() {
		...
		this.imageViews[0] = ((ImageView) findViewById(R.id.picture));
		this.imageViews[1] = ((ImageView) findViewById(R.id.video));
		this.imageViews[2] = ((ImageView) findViewById(R.id.web));
		this.imageViews[3] = ((ImageView) findViewById(R.id.setting));
		...
		FrameLayout.LayoutParams layoutparams = new FrameLayout.LayoutParams(160, 220);
	...
	public void onFocusChange(View paramView, boolean paramBoolean) {
		...
		case R.id.picture:
			...
			x = util.dip2px(context, 118);
			y = util.dip2px(context, 96);
			// 此处可封装出数学算法，计算偏移量的x,y值。我懒得算了，直接设置了
			break;
		case R.id.video:
			i = 1;
			x = util.dip2px(context, 364);
			y = util.dip2px(context, 96);
			break;
		case R.id.web:
			i = 2;
			x = util.dip2px(context, 624);
			y = util.dip2px(context, 96);
			break;
		case R.id.setting:
			i = 3;
			x = util.dip2px(context, 872);
			y = util.dip2px(context, 96);
			break;
		}
		if (paramBoolean) {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			flyWhiteBorder(width, height, x-70, y);
		}
	};
